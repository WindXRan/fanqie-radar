...
SERVER_INFO = {"name": "fanqie-index-mcp", "version": "0.1.0"}
...
def t_imitation_score(channel: str = "female", rank: str = "read", top: int = 30,
    ...
    for b in ranked:
        ...
        d["breakdown"] = {k: b.get(k) for k in
                          ("s_done", "s_words", "s_reads", "s_heat", "s_trope", "s_gf", "s_stable")}
...
TOOLS = [
    {
        "name": "fanqie_ranks",
        "description": "读取最新番茄榜单（按频道/榜型/品类）。返回书名、作者、在读、状态、字数、book_id——不含简介全文与版权图。",
        "inputSchema": {
            "type": "object",
            "properties": {
