...
3. **不向外部返回 `intro` 简介全文 / `cover` 版权图 URL。** 对外书目只含：book_id、标题、作者、品类、在读、状态、字数、url。
...
- **仿写选书评分 Top**：卡片网格，每卡含分数环 + 六维小条（完结/体量/热度/吸量/套路/金指）+ 套路/金手指命中标签
- **趋势差分**：新上榜 / 掉出榜单 两列（需 ≥2 份同榜快照）
- **书名热词**：套路词频标签云
- **榜单明细**：书名 / 作者 / 品类 / 在读 / 状态 / 字数 / 评分
